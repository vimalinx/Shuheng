...
- Scheduled-report excerpts must be user-readable final reply text. OMP/LLM process markers such as `LLM Running`, `<summary>`, `<thinking>`, tool payloads, approval ids, raw artifact URIs, and task-run ids stay out of the default visible text.
...
- Good: A scheduled subagent task completes, writes a subagent-result artifact, and then appears as a readable final-reply excerpt in both that subagent's home `定时汇报` section and the global `__home__:scheduled_reports` page without exposing raw artifact URIs or process/thinking blocks in the default text.
