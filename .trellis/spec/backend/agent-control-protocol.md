...
- Base: `split_table_row(...)`, `render_table(...)`, `markdown_blocks(...)`, `plain_blocks(...)`, and subagent result metadata parsing stay in `app.py`; they may call the `strip_inline_markdown(...)` compatibility alias while the implementation lives in `rendering.py`.
