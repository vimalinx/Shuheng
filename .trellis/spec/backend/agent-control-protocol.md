...
- `sanitize_interaction_candidates(...)`, `render_interaction_card(...)`, `interaction_answer_from_text(...)`, `compose_request_user_input_answer(...)`, and `interaction_input_prompt_text(...)` are deterministic interaction-display/input-text helpers over explicit raw candidate values, already-extracted interaction payload dictionaries, explicit selected indexes, explicit answer lists, or explicit prompt-state flags. They may trim numbered/quoted/list-like candidate labels, format the visible Chinese ask-user/request-user-input card, resolve numeric/free-text/blank-candidate answers, format multi-question `request_user_input` answer text, and return prompt labels such as `"> "`, `"approval> "`, `"qN> "`, and `"? "`. They must not parse tool payloads, call `process_tools(...)`, call `extract_interaction_request(...)`, inspect `State.pending_interaction`, read approval ledgers, mutate interaction selection, allocate `RenderLine`, choose curses attrs, or handle keyboard/input submission. App-owned `extract_interaction_request(...)`, approval interaction payloads, `normalize_interaction_payload(...)`, `interaction_current_candidates(...)`, `interaction_selection(...)`, `interaction_answer_from_input(...)`, `interaction_input_prompt(...)`, `interaction_hint_lines(...)`, and input handlers remain responsible for parsing, state, approval, current-question traversal, selection mutation, curses, and submission behavior.
...
- Interaction card/answer formatting with normal candidate prompts, approval prompts, multi-question `request_user_input` prompts, empty fallback payloads, numeric candidate answers, free-text answers, blank selected-candidate fallback, multi-question answer composition, and prompt labels -> returns the same visible text/answer strings as the legacy app helpers while leaving extraction/state/current-selection/input handling in `app.py`.
