...
- Plain text input on the main home starts the main Orchestrator task and switches to the main task/chat interface (`selected_session == "main"`). Plain text input on a persistent-agent home creates a fresh history-backed subagent chat session, auto-switches to that agent's chat interface, and sends the input as the first direct subagent chat turn. `/chat` from a persistent-agent home restores that agent's last entered chat session. Main home drill-downs stay command-driven through `/tasks`, `/schedules`, `/approvals`, and `/artifacts`.
...
- Good: Direct text on a persistent-agent home starts a fresh history-backed subagent chat session, while `/chat` from that home restores the previously entered session.
...
- Tests must assert direct text on a persistent subagent home creates a fresh history-backed subagent chat session; `/chat` or a subagent-session switch persists/restores the selected chat session id/ref from canonical history, not from a per-agent transcript store.
...
- Context-pack fields:
  ...
  - `transient_skill_pack.schema_version == "subagent.transient_skill_refs.v1"`
- Prompt label: `OMP native transient skill command refs for this prompt`.
...
- Prompt-level `$skill` never causes Shuheng to read or inject the transient skill body. The skill body is loaded only by OMP's own `/skill:<name>` command path when OMP has that skill in `session.skills`.
- Non-command-shaped transient refs stay visible in `transient_skill_refs` / `transient_skill_pack.included[].command_eligible:false` and are not converted into `/skill:<name>`.
...
- OMP native skill commands support one skill command at a time. When multiple transient refs are requested, Shuheng maps only the first command-safe ref to `/skill:<name>` and keeps all normalized refs in `transient_skill_refs` / metadata for audit.
- Shuheng must not add a fallback transient body pack if OMP does not load the skill. OMP's native loader is the source of truth for prompt-level `$skill`.
...
- `$huashu-info-search research topic` -> target task sees objective `research topic` plus `transient_skill_refs:["huashu-info-search"]`; if the runtime agent is OMP, the dispatched prompt starts with `/skill:huashu-info-search `.
- `$+huashu-info-search research topic` -> same normalized transient ref and objective as `$huashu-info-search research topic`.
- `@researcher $huashu-info-search research topic` -> dispatches one governed task to `researcher`, not the main Orchestrator.
- `/agent ask researcher $huashu-info-search research topic` -> dispatches one governed task to `researcher` with `/skill:huashu-info-search` at the start of the OMP prompt.
- Selected subagent chat `$custom-sop hello` -> the direct-chat OMP prompt starts with `/skill:custom-sop`; Shuheng context metadata records the ref only.
- Normal text `Please explain $PATH` -> no transient skill invocation.
- `$missing-skill do work` -> OMP receives `/skill:missing-skill ...`; if OMP does not have that skill, OMP owns the resulting behavior. Shuheng still injects no body text.
...
- `tests/test_commands.py` must assert leading-only transient parser behavior, `$+` compatibility, and `$` completion ranking with name matches before summary matches.
...
```text
$huashu-info-search task -> OMP receives /skill:huashu-info-search task; Shuheng records only transient_skill_refs metadata
```
...
## Scenario: OMP Native Runtime Query Host Tools
...
- Compatibility query endpoint names:
  - `shuheng_query({"endpoint":"runtime_subagent_list","args":{...}})`
  - `shuheng_query({"endpoint":"runtime_subagent_messages","args":{...}})`
- Typed read-only tool names:
  - `runtime_subagent_list`
  - `runtime_subagent_messages`
- Runtime message args:
  - `subagent_id`: optional OMP-native runtime subagent id.
  - `session_file`: optional OMP-native runtime subagent session file.
  - `from_byte`: optional byte offset for incremental transcript reads.
...
- `agent_list` remains the Shuheng-managed worker list from TUI state; it must not call OMP native runtime methods.
- `runtime_subagent_list` and `runtime_subagent_messages` are OMP-native runtime facts. They must call the current active runtime agent's provider methods when those methods exist.
- `install_interaction_hook(...)` must bind `runtime_agent=agent` into the host-tool handler so a tool call emitted by a live OMP process queries that same live process.
...
- `runtime_subagent_list` with no `state` and no `runtime_agent` -> `shuheng.query.v1` error saying an active runtime agent is required.
- Active runtime object lacks `get_runtime_subagents` -> `shuheng.query.v1` error with `kind:"runtime.subagent.list"`.
- Active runtime object lacks `get_runtime_subagent_messages` -> `shuheng.query.v1` error with `kind:"runtime.subagent.messages"`.
...
- `scripts/check_policy_gates.py` must assert `shuheng_query` endpoint enum includes `runtime_subagent_list` and `runtime_subagent_messages`.
- `scripts/check_policy_gates.py` must assert typed host tools include `runtime_subagent_list` and `runtime_subagent_messages` plus the `subagent_id`, `session_file`, and `from_byte` parameters.
- Tests must assert `runtime_subagent_list` calls a fake active runtime agent's `get_runtime_subagents()`.
...
```text
runtime_subagent_list -> read State.subagents -> return those rows as OMP runtime subagents.
```
