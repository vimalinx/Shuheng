...
- Interaction card text ownership: `rendering.sanitize_interaction_candidates(raw)` and `rendering.render_interaction_card(payload)` own deterministic ask-user/request-user-input visible card text over explicit payloads. `app.py` keeps interaction request extraction, pending interaction state, approval payload construction, candidate selection, answer submission, curses hint rows, and input handling.
...
- `sanitize_interaction_candidates(...)` and `render_interaction_card(...)` are deterministic interaction-display helpers over explicit raw candidate values or already-extracted interaction payload dictionaries. They may trim numbered/quoted/list-like candidate labels and format the visible Chinese ask-user/request-user-input card, but must not parse tool payloads, call `process_tools(...)`, call `extract_interaction_request(...)`, inspect `State.pending_interaction`, read approval ledgers, mutate interaction selection, allocate `RenderLine`, choose curses attrs, or handle keyboard/input submission. App-owned `extract_interaction_request(...)`, approval interaction payloads, `normalize_interaction_payload(...)`, `interaction_hint_lines(...)`, and input handlers remain responsible for parsing, state, approval, curses, and submission behavior.
...
- Interaction card formatting with normal candidate prompts, approval prompts, multi-question `request_user_input` prompts, and empty fallback payloads -> returns the same visible card text as the legacy app helper while leaving extraction/state/input handling in `app.py`.
