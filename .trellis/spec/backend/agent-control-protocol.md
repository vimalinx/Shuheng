...
- Process-line formatter ownership: `rendering.process_turn_label(marker)`, `rendering.process_tool_suffix(tools)`, `rendering.process_turn_no(marker, fallback)`, `rendering.collapsed_process_line_text(...)`, `rendering.process_detail_line_text(...)`, `rendering.process_speech_header_text(...)`, `rendering.process_speech_summary_line_text(...)`, `rendering.process_summary_append_lines(summary, summary_line)`, `rendering.expanded_process_header_text(...)`, `rendering.process_group_header_text(...)`, `rendering.collapsed_process_child_line_text(...)`, and `rendering.expanded_process_child_header_text(...)` own deterministic process row/header string formatting and summary-row append policy over explicit values. `app.py` keeps the legacy process-row functions as wrappers that inject app-owned process title, summary, and tool parsing, and keeps `append_process_summary_line(rendered, marker, body)` as the wrapper that mutates the caller-provided rendered list.
...
- Process-line formatter helpers preserve legacy Chinese process row strings, turn-number fallback parsing, tool suffix truncation with `+N`, group expand/collapse status labels, child row label insertion, and summary-row append policy while receiving already-parsed title/summary/tool data or already-formatted summary rows from `app.py`.
