...
- `table_layout_lines(...)` and `markdown_layout_blocks(...)` return neutral layout records over already-loaded markdown text. `table_layout_lines(...)` owns column sizing and table row records, while `markdown_layout_blocks(...)` owns deterministic block parsing for code fences, tables, blank lines, rules, headings, quotes, task items, bullets, numbered items, and body text. They may use lower-level text wrapping/inline cleanup helpers, but must not allocate `RenderLine`, choose curses attrs, inspect `State`, read or mutate message caches, parse process/tool payloads, call Web Console/dashboard/runtime/input/command owners, or perform storage/runtime side effects. App-owned `render_table(...)` and `markdown_blocks(...)` remain compatibility wrappers that convert neutral records into existing `RenderLine` values with `cp(...)` and curses attrs.
...
- Base: `is_table_separator(...)`, `split_table_row(...)`, and `table_layout_lines(...)` are rendering-owned parser/layout helpers re-exported by `app.py`; `render_table(...)` stays in `app.py` as the compatibility wrapper that converts neutral layout records into `RenderLine` values with curses attrs, while `markdown_blocks(...)`, `plain_blocks(...)`, and subagent result metadata parsing stay in `app.py`.
