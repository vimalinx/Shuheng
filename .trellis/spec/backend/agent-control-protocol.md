...
- Base: `process_title_text(...)`, `process_tools(...)`, process grouping/folding, `message_block_lines(...)`, and `message_lines_from_cache(...)` stay in `app.py` until their JSON-ish parsing, search-noise detection, mutable cache, and curses/render-line dependencies have clean boundaries. Process-line formatter helpers can live in `rendering.py` only because `app.py` injects the parsed title/summary/tool values.
