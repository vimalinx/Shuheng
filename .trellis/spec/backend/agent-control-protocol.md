...
- Subagent store helpers: `subagent_store.subagent_home(...)`, `subagent_store.subagent_*_path(...)`, `subagent_store.subagent_session_sidebar_key(...)`, `subagent_store.subagent_session_from_sidebar_key(...)`, `subagent_store.subagent_chat_history_meta_matches(...)`, and pure subagent chat title/preview/description/count helpers, with `app.py` wrappers injecting app-owned roots such as `SUBAGENTS_DIR` or runtime objects such as `SubAgentRuntime`.
