...
Continue Goal 7 by extracting the deterministic markdown table layout calculation from `src/shuheng/app.py` into the lower-level curses-free rendering boundary in `src/shuheng/rendering.py`.
...
- Keep markdown table parsing helpers `split_table_row(...)` and `is_table_separator(...)` in `src/shuheng/rendering.py`.
- Keep `src/shuheng/app.py::render_table(...)` as the public compatibility wrapper that converts neutral layout records into `RenderLine` objects with existing `cp(...)` and curses attrs.
