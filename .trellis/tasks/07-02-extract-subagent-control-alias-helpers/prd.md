...
Move deterministic subagent control alias lookup helpers out of `src/ga_tui/app.py` into the lower-level `src/ga_tui/subagent_store.py` boundary while preserving current behavior and public compatibility aliases.
...
- Move `subagent_control_alias_keys(*values)` into `src/ga_tui/subagent_store.py`.
- Move `resolve_subagent_control_alias(alias_map, target)` into `src/ga_tui/subagent_store.py`.
- Keep `src/ga_tui/app.py` exposing the same public helper names as compatibility aliases.
- Keep `register_subagent_control_aliases(alias_map, sub, *values)` in `src/ga_tui/app.py` as the Orchestrator wrapper because it depends on `SubAgentRuntime`.
...
`subagent_store.py` may own pure identity/ref/alias shaping over explicit values and maps. It must not import `ga_tui.app`, curses, mutable `State`, runtime dispatch, command handlers, rendering, Web Console, ledgers, approvals, artifacts, or storage root globals.
