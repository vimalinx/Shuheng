...
Continue decomposing `src/ga_tui/app.py` by extracting the next low-risk part of runtime dispatch into `src/ga_tui/runtime_dispatch.py`: provider identity helpers, Oh My Pi runtime metadata readers, provider-neutral `RuntimeTaskRequest` construction, and the small compatibility `put_agent_runtime_task(...)` wrapper.
...
- The runtime request/event dataclasses already live in `src/ga_tui/runtime.py`.
...
- Extracted modules must not import `ga_tui.app`, `curses`, UI renderers, command handlers, `State`, or `SubAgentRuntime`.
...
- Create `src/ga_tui/runtime_dispatch.py` as a lower-level helper module.
...
- [ ] `src/ga_tui/runtime_dispatch.py` exists and contains the moved provider-neutral helpers.
- [ ] `runtime_dispatch.py` does not import `ga_tui.app`, `.app`, `app`, `curses`, UI renderers, `State`, or `SubAgentRuntime`.
...
- Current helper cluster starts near `src/ga_tui/app.py` around `agent_runtime_provider_id(...)`.
