...
Continue decomposing `src/shuheng/app.py` by moving deterministic process-group header title/tool aggregation into the curses-free `src/shuheng/rendering.py` helper layer, while preserving existing process rendering behavior and keeping the Orchestrator facade responsible for process-body parsing.
...
- Keep `src/shuheng/app.py` as the compatibility facade and owner of `process_summary_text(...)`, `process_tools(...)`, JSON-ish tool parsing, process turn traversal, `RenderLine` allocation, `cp(...)`, curses attrs, message rendering, and mutable UI state.
...
- [ ] `src/shuheng/rendering.py` owns a deterministic process-group header aggregation helper.
- [ ] `src/shuheng/app.py` keeps process summary/tool extraction and only passes explicit summaries/tool lists into the helper.
...
- Current function: `src/shuheng/app.py::process_group_header(...)`.
- Existing formatter owner: `src/shuheng/rendering.py::process_group_header_text(...)`.
