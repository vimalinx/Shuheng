...
- Extracted modules must not import `shuheng.app`, `curses`, UI renderers, or command handlers.
- `app.py` must keep compatibility wrappers because tests/scripts still import many helpers from `shuheng.app`.
...
- Create `src/shuheng/context_packs.py` for context-pack/domain helpers that are below `app.py`.
...
- [ ] `src/shuheng/context_packs.py` exists and contains extracted context-pack helpers.
- [ ] `context_packs.py` does not contain `shuheng.app`, `from .app`, `import app`, `import curses`, or `from curses`.
