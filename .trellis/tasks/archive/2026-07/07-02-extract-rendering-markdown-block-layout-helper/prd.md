...
Move deterministic markdown block layout from `src/shuheng/app.py` into the lower-level, curses-free `src/shuheng/rendering.py` module while preserving existing `app.markdown_blocks(text, width)` behavior through a compatibility wrapper.

## Scope

- Add a neutral markdown layout helper in `src/shuheng/rendering.py`.
...
- Keep `src/shuheng/app.py` responsible for converting layout records into `RenderLine` instances with the existing `cp(...)` and curses attrs.
...
- Do not import `shuheng.app`, `curses`, mutable `State`, Web Console, dashboard, runtime dispatch, command handlers, input handlers, storage roots, ledgers, approvals, artifacts, Secret Vault behavior, or history ownership into `rendering.py`.
- Do not change markdown rendering semantics beyond the mechanical extraction.

## Acceptance Criteria

- `src/shuheng/rendering.py` owns `markdown_layout_blocks(text, width)`.
- `markdown_layout_blocks(...)` returns neutral records with stable kind strings that let `app.py` reproduce existing attrs.
- `src/shuheng/app.py` exposes `markdown_layout_blocks` as a compatibility alias and keeps `markdown_blocks(...)` as a wrapper.
