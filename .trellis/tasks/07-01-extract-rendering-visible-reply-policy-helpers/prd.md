...
Move the remaining pure visible-reply policy helpers out of `src/ga_tui/app.py` and into `src/ga_tui/rendering.py` while preserving all existing behavior and app-level compatibility names.
...
The implementation should keep `src/ga_tui/app.py` as a compatibility facade by importing or aliasing the same public names from `src/ga_tui/rendering.py`.
...
- Do not introduce `ga_tui.app`, curses, mutable `State`, runtime dispatch, Web Console, dashboard, command/input handlers, storage roots, approvals, artifacts, history ownership, or task-ledger dependencies into `rendering.py`.

## Compatibility

- Existing imports from `ga_tui.app` must continue to work.
