...
Continue the `src/ga_tui/app.py` decomposition by moving the deterministic subagent result card layout decisions into the curses-free `src/ga_tui/rendering.py` helper layer, while preserving the existing TUI behavior and keeping the Orchestrator facade responsible for message traversal, rendering dependency injection, and curses `RenderLine` assembly.

## Requirements

- Add a pure rendering helper for subagent result card string/layout decisions in `src/ga_tui/rendering.py`.
- Keep `src/ga_tui/app.py` as the compatibility facade and owner of `RenderLine` allocation, `cp(...)`, curses attributes, `render_subagent_result_body(...)`, `markdown_blocks(...)`, `plain_blocks(...)`, and `subagent_result_metadata_detail_blocks(...)`.
...
- [ ] `src/ga_tui/rendering.py` owns a pure helper for subagent result card layout records or text decisions.
- [ ] `src/ga_tui/app.py` keeps all `RenderLine`, `cp(...)`, curses, markdown/plain conversion, metadata detail block, and rendered-body injection responsibilities.
...
- Current function: `src/ga_tui/app.py::subagent_result_card_blocks(...)`.
- Existing helper owner: `src/ga_tui/rendering.py`.
