...
Continue Goal 7 by extracting deterministic process-line formatting helpers from `src/shuheng/app.py` into the curses-free `src/shuheng/rendering.py` module, while preserving existing public compatibility names in `app.py` and avoiding any behavior rewrite.
...
- Do not add a reverse import from `rendering.py` to `shuheng.app`.

## Requirements

1. `src/shuheng/rendering.py` owns pure helper functions for process-line string formatting.
2. `src/shuheng/app.py` preserves current public function names and output behavior via compatibility wrappers or aliases.
3. `rendering.py` remains a lower-level dependency with no `shuheng.app`, curses, mutable TUI `State`, runtime dispatch, command handler, Web Console, dashboard, input handler, storage-root, or ledger dependency.
