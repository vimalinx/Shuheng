...
Move pure Secret Vault value/payload helper logic out of `src/ga_tui/app.py` into `src/ga_tui/secret_vault.py`, while preserving existing behavior through `app.py` compatibility aliases or thin wrappers.
...
- Do not make `secret_vault.py` import `ga_tui.app`, curses, mutable TUI `State`, runtime providers, Web Console, dashboard, command handlers, or rendering owners.
