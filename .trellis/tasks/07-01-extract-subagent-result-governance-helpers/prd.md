...
Move pure subagent-result task-ledger helper logic out of `src/shuheng/app.py` into `src/shuheng/governance.py`, while keeping `app.py` compatibility wrappers for app-owned artifact-path and subagent-meta dependencies.
...
- Do not introduce a `governance.py` dependency on `shuheng.app`, curses, `State`, `PanelItem`, `RenderLine`, or draw/command handlers.

## Verification

- `python3 -m py_compile src/shuheng/app.py src/shuheng/governance.py tests/test_governance.py scripts/check_policy_gates.py`
- `python3 -m ruff check src/shuheng/app.py src/shuheng/governance.py tests/test_governance.py scripts/check_policy_gates.py`
