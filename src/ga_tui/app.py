...
def build_main_runtime_context_pack(
    ...
    main_worker = SubAgentRuntime(
        ...
        status=state.status,
        profile_text=(
            "Primary Shuheng Orchestrator. It owns planning, routing, policy gates, "
...
def recent_model_entry_indices(entries: list[LLMConfigEntry], recent_names: list[str]) -> list[int]:
    indices: list[int] = []
    seen: set[int] = set()
...
def next_recent_entry_index(entries: list[LLMConfigEntry], recent_names: list[str], selected: int) -> int:
    if not entries or not recent_names:
        return selected
    indices = recent_model_entry_indices(entries, recent_names)
    if not indices:
        return selected
    for idx in indices:
        if idx > selected:
...
def save_llm_config_entries(entries: list[LLMConfigEntry], mixin: dict[str, Any], preserved: list[tuple[str, Any]]) -> tuple[bool, str]:
    ...
    lines = [
        "# Generated by Shuheng model manager.",
        "# Edit/switch in TUI with /model.",
        "",
        "mixin_config = {",
    ]
...
def set_agent_to_model_name(agent: Any, model_name: str) -> tuple[bool, str]:
    ...
    if index < 0:
        return False, f"当前运行时未找到模型配置：{model_name}。请先用 /model 按 r 重载配置。"
...
def ohmypi_typed_governed_host_tool_definitions() -> list[RpcHostToolDefinition]:
    return [
        ...
        RpcHostToolDefinition(
            ...
            description="Create a Shuheng scheduled task record through the control-plane scheduler service.",
            parameters={
                "type": "object",
                "additionalProperties": True,
                "properties": {
...
def draw_model_manager(
    ...
    *,
    ...
    try:
        ...
        selected_row = 0
    view_start = max(0, min(selected_row - list_h + 1, max(0, len(visible_indices) - list_h)))
    for row, idx in enumerate(visible_indices[view_start:view_start + list_h]):
        entry = entries[idx]
        cfg = entry.cfg
        name = config_display_name(entry)
    ...
    try:
        ...
        while True:
            ...
            if key in ("\x1b", 27, "q", "Q"):
                break
            left_category_keys = (getattr(curses, "KEY_BTAB", -987654), curses.KEY_LEFT, "[", "{")
            if key in ("\t", curses.KEY_RIGHT, "]", "}") or key in left_category_keys:
                direction = -1 if key in left_category_keys else 1
                if len(categories) <= 1:
                    message = f"当前只有 {active_category} 分类。"
                    continue
                try:
                    category_pos = categories.index(active_category)
                except ValueError:
                    category_pos = 0
            ...
            if key in (curses.KEY_UP,):
                if visible_indices:
                    pos = visible_indices.index(selected) if selected in visible_indices else 0
                    selected = visible_indices[(pos - 1) % len(visible_indices)]
                else:
                    selected = 0
                continue
            if key in (curses.KEY_DOWN,):
            ...
            if manage_configs and entries and key in ("e", "E"):
                ...
                active_category = model_entry_category(edited)
                recent_names = load_recent_model_names(entries)
            ...
            if key in ("v", "V"):
                if not entries:
                    message = "没有可验活的模型；先用 /model 添加供应商/API。"
                    continue
                selected_key = model_health_key(entries[selected])
                total = len(entries)
                done = 0
...
def submit(state: State, text: str) -> None:
    ...
    if text == "/schedules":
        ...
        return
    m_scheduler = re.match(r"/scheduler(?:\s+(.*))?\s*$", text, re.I)
    if m_scheduler:
        args = (m_scheduler.group(1) or "").strip()
        if not args or args == "status":
            data = scheduled_task_registry(state)
            add_system(state, "Scheduler 状态：\n" + format_scheduled_task_registry(data))
            return
        if args == "tick":
            result = scheduler_tick(state, source="manual:scheduler_tick", record_skips=True)
            add_system(state, format_scheduler_tick_result(result))
            return
        m_run = re.match(r"run\s+(\S+)\s*$", args, re.I)
        if m_run:
            target_id = m_run.group(1)
            if target_id not in latest_schedule_records():
                add_system(state, f"找不到定时任务：{target_id}")
                return
            result = scheduler_tick(state, source="manual:scheduler_run", target_schedule_id=target_id, force=True, record_skips=True)
            add_system(state, format_scheduler_tick_result(result))
            return
...
def init_curses(stdscr) -> None:
    ...
    try:
        ...
        if curses.COLORS >= 16:
            ...
            curses.init_pair(12, 8, curses.COLOR_BLACK)
        else:
            curses.init_pair(1, curses.COLOR_CYAN, curses.COLOR_BLACK)
            curses.init_pair(2, curses.COLOR_WHITE, curses.COLOR_BLACK)
            curses.init_pair(3, curses.COLOR_GREEN, curses.COLOR_BLACK)
...
def run(stdscr) -> dict[str, Any]:
    ...
    try:
        ...
        next_scheduler_tick = time.monotonic() + min(5.0, SCHEDULER_TICK_SECONDS)
        while state.running:
            process_ui_queue(state)
            now = time.monotonic()
            if now >= next_scheduler_tick:
                schedule_result = scheduler_tick(state, source="daemon:scheduler_tick")
                if schedule_result.get("runs") or int(schedule_result.get("due") or 0) or int(schedule_result.get("failed") or 0):
                    add_system(state, format_scheduler_tick_result(schedule_result), persist=True, kind="scheduler_tick")
                next_scheduler_tick = now + SCHEDULER_TICK_SECONDS
