...
`scripts/wheel_smoke.py --dist-dir` 默认会分别安装最新 wheel 和 sdist，检查 wheel metadata/私有文件边界与 sdist archive 的公开/私有文件边界，并运行公开入口点检查；`--no-deps` 和 `--wheel-only` 仅用于本地调试，不属于发布门禁。
