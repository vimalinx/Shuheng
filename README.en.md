...
`scripts/wheel_smoke.py --dist-dir` installs both the latest wheel and sdist, checks the sdist archive public/private file boundary, and runs public entrypoint checks. `--no-deps` and `--wheel-only` are local debugging options, not release gates.
